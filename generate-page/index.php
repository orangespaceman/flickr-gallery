<?php
  ...
  if (!$conf['flickr']['api_key'] || !$conf['flickr']['api_secret']) {
    ...
    debug("visit: https://www.flickr.com/services/apps/create/");
    exit;
  }
  ...
  // step 1: https://www.flickr.com/services/auth/?api_key=[]&perms=read&api_sig=[]
  if (!$conf['flickr']['frob']) {
    $api_sig = md5($conf['flickr']['api_secret']."api_key".$conf['flickr']['api_key']."permsread");
    $frobURL = "https://www.flickr.com/services/auth/?api_key=".$conf['flickr']['api_key']."&perms=read&api_sig=".$api_sig;
    debug("error: no frob");
    debug("visit: ".$frobURL);
    exit;
  }
  ...
  if (!$conf['flickr']['auth_token']) {
    ...
    $tokenURL = "https://api.flickr.com/services/rest/?method=flickr.auth.getToken&api_key=".$conf['flickr']['api_key']."&frob=".$conf['flickr']['frob']."&format=json&nojsoncallback=1&api_sig=".$api_sig;
  ...
  $collectionsURL = "https://api.flickr.com/services/rest/?method=flickr.photosets.getList&api_key=".$conf['flickr']['api_key']."&auth_token=".$conf['flickr']['auth_token']."&format=json&nojsoncallback=1&api_sig=".$api_sig;
  ...
  foreach($collections->photosets->photoset as $photoset) {
    ...
    $photosURL = "https://api.flickr.com/services/rest/?method=flickr.photosets.getPhotos&api_key=".$conf['flickr']['api_key']."&photoset_id=".$photoset->id."&extras=description&format=json&nojsoncallback=1&auth_token=".$conf['flickr']['auth_token']."&api_sig=".$api_sig;
